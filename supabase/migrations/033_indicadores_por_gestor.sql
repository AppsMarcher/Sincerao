-- Adiciona "Resultados por gestor" aos Indicadores de Gestão, no mesmo
-- formato de "Resultados por setor" (migration 017). create or replace
-- function porque a CTE "base" precisa ganhar gestor_nome.
...
begin
  ...
  por_gestor as (select coalesce(gestor_nome,'Sem gestor') gestor,count(*)::int total,count(*) filter(where status='concluida')::int concluidas,round(avg(pontuacao_geral) filter(where status='concluida'),2) nota_media from base group by gestor_nome order by gestor),
