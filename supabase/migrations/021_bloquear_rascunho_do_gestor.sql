-- O rascunho (Fase 1) pertence exclusivamente ao gestor até o envio ao colaborador.
...
create policy avaliacoes_select on avaliacoes for select using (
  ...
  or (colaborador_id = auth.uid()
      and status not in ('rascunho', 'aguardando_autoavaliacao')
    ...
    where a.id = avaliacao_notas.avaliacao_id
      and (
        ...
        or (a.colaborador_id = auth.uid()
            and a.status not in ('rascunho', 'aguardando_autoavaliacao')
            and ciclo_vigente(a.ciclo_id))
        or sou_rh_ou_admin()
      )
  )
);
